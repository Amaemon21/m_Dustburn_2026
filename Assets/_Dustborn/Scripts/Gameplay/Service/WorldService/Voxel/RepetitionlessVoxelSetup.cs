...
public static class RepetitionlessVoxelSetup
{
    ...
    private const string NOISE_TEXTURE_KEYWORD = "_REPETITIONLESS_NOISE_TEXTURE";
    private const string NOISE_TEXTURE_PROPERTY = "_NoiseTexture";

    private const float WORLD_UV_SCALE = 1000f;
    private const float NOISE_CELL_METRES = 24f;
    ...
    private static void WarnAboutMissingNormals(TerrainLayer[] layers, Material material)
    {
        ...
        Debug.LogError($"These terrain layers have no normal map: {missing}. Repetitionless packs one normal slice per layer, and a hole in that sequence throws ArgumentOutOfRangeException out of its texture array drawer, leaving the layer unpainted. Assign a normal map to every layer", material);
    }

    private static void UseDynamicNoise(RepetitionlessMaterialDataSO properties, Material material)
    {
        material.DisableKeyword(NOISE_TEXTURE_KEYWORD);
        material.SetInt(NOISE_TEXTURE_KEYWORD, 0);
        material.SetTexture(NOISE_TEXTURE_PROPERTY, null);

        if (properties != null)
            properties.SetNoiseQuality(ENoiseQuality.High);
    }
    ...
    private static void ApplyWorldTiling(RepetitionlessMaterialDataSO properties, Material material, TerrainLayer[] layers, int capacity)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            properties.Data[i].BaseMaterialData.NoiseScale = NoiseCells(size);
    ...
    private static float NoiseCells(Vector2 tileSize)
    {
        float metres = Mathf.Max(tileSize.x, tileSize.y);

        if (metres <= 0f)
            return 1f;

        return metres / NOISE_CELL_METRES;
    }
}
#endif

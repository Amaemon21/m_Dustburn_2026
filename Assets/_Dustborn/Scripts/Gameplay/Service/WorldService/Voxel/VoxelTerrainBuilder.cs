...
public class VoxelTerrainBuilder : MonoBehaviour
{
    ...
    private int MeshChunks(VoxelDensityField field, List<ChunkTask> tasks, System.Action<string, float> progress)
    {
        ...
        var meshers = new VoxelChunkMesher[workers];
        var meshes = new VoxelMesh[workers];
        var handles = new JobHandle[workers];
        ...
        try
        {
            ...
            for (int start = 0; start < tasks.Count; start += workers)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    handles[i] = meshers[i].Schedule(task.Key.Lod, task.Key.X, task.Y, task.Key.Z, meshes[i], task.Key.Seams, task.Key.Morph);
                }
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    ChunkTask task = tasks[start + i];
                    triangles += meshes[i].TriangleCount;
                    Spawn(meshes[i], task.Column, $"Chunk {task.Key} y {task.Y}", colliders);
                }
            }

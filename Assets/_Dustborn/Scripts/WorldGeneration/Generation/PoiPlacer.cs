...
public class PoiPlacer
{
    ...
    private readonly PoiCensus _reserved = new();
    private readonly PoiCensus _settlementReserved = new();

    private int _skippedSteep;
    private int _skippedNoFit;
    private int _skippedOnRoad;
    private int _lostRequired;
    ...
    public List<PoiPlacement> Place(IReadOnlyList<CityLayout> layouts, RoadNetwork network)
    {
        _placements.Clear();
        ...
        {
            _settlement.Clear();
            _settlementReserved.Clear();

            foreach (Lot lot in layout.Lots)
                _settlementReserved.Add(lot.Reserved);
            ...
            foreach (Lot lot in layout.Lots)
            {
                if (lot.Reserved != null)
                    PlaceOnLot(lot, layout.Profile, ref random);
            }
        ...
        int inCities = _placements.Count;
    ...
    private void CountReserved(IReadOnlyList<CityLayout> layouts)
    {
        foreach (CityLayout layout in layouts)
        {
            foreach (Lot lot in layout.Lots)
                _reserved.Add(lot.Reserved);
        }
    }

    private void PlaceOnLot(Lot lot, SettlementProfile profile, ref Random random)
    {
        PoiDefinition definition = lot.Reserved;

        if (definition != null)
        {
            _reserved.Add(definition, -1);
            _settlementReserved.Add(definition, -1);
        }
        else
        {
            definition = Pick(lot.District, lot.Width - 2f * profile.LotMargin, lot.Depth - _config.LotSetback - profile.LotMargin, ref random);
        }
    ...
    private void PlaceAlongRoads(RoadNetwork network, ref Random random)
    {
        ...
        foreach (Road road in network.Roads)
        {
            ...
            for (int i = 0; i < road.Points.Length - 1; i++)
            {
                ...
                if (IsInsideCity(network, position))
                    continue;
    ...
    private bool IsInsideCity(RoadNetwork network, Vector2 position)
    {
        foreach (Hub hub in network.Hubs)
        {
            float radius = hub.Radius * _config.CityRadiusScale * _config.RuralClearance;
    ...
    {
        if (definition.MaxPerWorld > 0 && _world.Count(definition) + _reserved.Count(definition) >= definition.MaxPerWorld)
            return false;

        if (definition.MaxPerSettlement <= 0)
            return true;

        return _settlement.Count(definition) + _settlementReserved.Count(definition) < definition.MaxPerSettlement;
    }
    ...
    private void Report(int lots, int inCities)
    {
        if (_placements.Count == 0)
        {
            Debug.LogWarning($"Not a single POI was placed across {lots} lots. Check the PoiDefinition footprints against the block size and raise MaxPoiCut and MaxPoiFill");

            return;
        }

        Debug.Log($"POI: {_placements.Count} placed, {inCities} in settlements, {_placements.Count - inCities} along roads");

        if (_lostRequired > 0)
            Debug.LogWarning($"POI: {_lostRequired} required buildings lost their reserved lot to the terrain or a road. Raise MaxPoiCut and MaxPoiFill or widen the radius band of that requirement");
        ...
        if (_skippedSteep > 0)
            Debug.Log($"POI: {_skippedSteep} lots dropped by the terrain. Raise MaxPoiCut and MaxPoiFill or lower CityRadiusScale");

        if (_skippedOnRoad > 0)
            Debug.Log($"POI: {_skippedOnRoad} lots dropped for landing on the highway surface. That is expected, the highway runs straight through the settlement");
    }
}

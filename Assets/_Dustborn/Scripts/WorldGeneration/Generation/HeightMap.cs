...
public class HeightMap
{
    ...
    public static HeightMap FromRaw16(byte[] bytes, int resolution, int worldSize, float maxHeight)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            map._heights[i] = value / (float)ushort.MaxValue;
        }
    ...
    public byte[] ToRaw16()
    {
        byte[] bytes = new byte[_heights.Length * 2];

        for (int i = 0; i < _heights.Length; i++)
        {
            ushort value = (ushort)Mathf.Clamp(Mathf.RoundToInt(_heights[i] * ushort.MaxValue), 0, ushort.MaxValue);

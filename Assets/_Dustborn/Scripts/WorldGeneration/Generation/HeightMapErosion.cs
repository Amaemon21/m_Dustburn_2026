...
public static class HeightMapErosion
{
    public static void Run(WorldGenerationConfig config, NativeArray<float> heights, int resolution)
    {
        ...
        var loss = new NativeArray<float>(heights.Length, Allocator.TempJob, NativeArrayOptions.UninitializedMemory);
        var scale = new NativeArray<float>(heights.Length, Allocator.TempJob, NativeArrayOptions.UninitializedMemory);
        ...
        try
        {
            ...
            for (int pass = 0; pass < config.ErosionPasses; pass++)
            {
                ...
                var settle = new ErosionSettleJob
                {
                    Heights = source,
                    Loss = loss,
                    Scale = scale,
                ...
                settle.Schedule(resolution, 1, flow.Schedule(resolution, 1)).Complete();
